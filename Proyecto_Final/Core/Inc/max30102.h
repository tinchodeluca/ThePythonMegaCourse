...
typedef struct MAX30102_REG{
	uint8_t SAMP_AVER;
	uint8_t FIFO_ROLLOVER; //BOOLEAN
	uint8_t FIFO_A_FULL;   //SAMPLES TO MAKE FIFO FULL
	uint8_t LED_RATE;
	uint8_t PULSE_WIDH;
	float LED_IR_I;
	float LED_RED_I;
...
I2C_HandleTypeDef _ui2c_max;
uint8_t _interrupt_flag;

#define MAX30102_I2C_ADDR 					0x57
#define MAX30102_I2C_TIMEOUT 				HAL_MAX_DELAY
...
uint8_t max30102_has_interrupt();
...
max30102_data max30102_getdata (void);
void max30102_write_byte( uint8_t reg, uint8_t buf);

uint8_t max30102_read( uint8_t reg, uint8_t *buf, uint16_t buflen);
void max30102_read_temp( int8_t *temp_int, uint8_t *temp_frac);
